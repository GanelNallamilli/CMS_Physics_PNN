...
hist_range=(80,180) #def histogram range
hist_range=None
feature='ditau_dR' #choose feature to plot
...
event_features=[['Diphoton_pt_mgg',
'Diphoton_dPhi',
'LeadPhoton_pt_mgg',
...
# =============================================================================
    ...
    plt.scatter(FPR_arr2,TPR_arr2)
    plt.scatter(FPR_arr3,TPR_arr3)
    plt.plot(FPR_arr1,TPR_arr1,label=f'AUC:{round(AUCscore1,4)} {signal1}')
    plt.plot(FPR_arr2,TPR_arr2,label=f'AUC:{round(AUCscore2,4)} {signal2}')
    plt.plot(FPR_arr3,TPR_arr3,label=f'AUC:{round(AUCscore3,4)} {signal3}')
    plt.xlabel('FPR')
    plt.ylabel('TPR')
    plt.title(f'ROC Curves for {feature}')
...
#%%
listofAUCst=listofAUCs
arrangingAUCs=np.transpose(np.array(listofAUCst).reshape(16,4))


# Zip the second row with other rows to maintain their association
zipped_data = list(zip(*arrangingAUCs))

# Sort based on the values of the second row (descending order)
sorted_zipped_data = sorted(zipped_data, key=lambda x: float(x[3]))

# Unzip the sorted data to separate the rows
sorted_data = list(zip(*sorted_zipped_data))

print(sorted_data)

figure=plt.figure
plt.scatter(sorted_data[0],sorted_data[1],label=f'{signal1}')
plt.scatter(sorted_data[0],sorted_data[2],label=f'{signal2}')
plt.scatter(sorted_data[0],sorted_data[3],label=f'{signal3}')

plt.plot(sorted_data[0],sorted_data[1])
plt.plot(sorted_data[0],sorted_data[2])
plt.plot(sorted_data[0],sorted_data[3])
plt.xticks(rotation=90,fontsize=10)
plt.yticks(fontsize=10)
plt.legend()
plt.xlabel('Event Features')
plt.ylabel('AUCs')
plt.title('Plot of Event Features Against AUC scores')
plt.show()